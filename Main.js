...
function fillCard2(arr, oid, cont){
	...
	var arrC = arr;
	var rows = orm("gT2",[arrC, [],/*[arrC.length-1],*/[],false,decorateArr(arrC, "`").concat(decorateArr(arrC, "`id_", "`"))/*.join(",")*/,
...
function fillCardEasy(arr, id, cont){
	...
	var arrC = arr;
	var filesInd = arrC.indexOf("Файлы");
	var rows = orm("gT2",[arrC,[],[],false,decorateArr(arrC, "`")," and `id_"+arrC[0]+"`="+id+" order by "+decorateArr(arrC, "`id_", "`").join(",")]);
	var tb = cont.appendChild(cDom("TABLE"));
